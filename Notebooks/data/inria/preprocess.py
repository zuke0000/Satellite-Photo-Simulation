...
def patch_inria(data_dir, save_dir, patch_size, pad, overlap):
    ...
    for city_name in tqdm(SAVE_CITY, desc='City-wise'):
        tutorial_range = 37 + (city_name=='chicago') # chicago 37 is the simulation
        #if (city_name=='chicago'):
        #    pass
        for tile_id in tqdm(range(1, tutorial_range), desc='Tile-wise', leave=False):
            ...
            for rgb_patch, gt_patch, y, x in data_utils.patch_tile(rgb_filename, gt_filename, patch_size, pad, overlap):
                ...
                misc_utils.save_file(os.path.join(patch_dir, gt_patchname), (np.ceil(gt_patch/255)).astype(np.uint8))
...
if __name__ == '__main__':
    ...
    pd = 0
    ol = 0
    save_dir = os.path.join(f'{os.getcwd()}/inria_save', 'ps{}_pd{}_ol{}'.format(ps, pd, ol))
    misc_utils.make_dir_if_not_exist(save_dir)
    patch_inria(data_dir=f'{os.getcwd()}',
                save_dir=save_dir,
                patch_size=(ps, ps),
                pad=pd, overlap=ol)
